...
export function registerQueryPageTool(server: McpServer) {
  server.tool(
    "query_page",
    "Inspects the current page. Modes: 'map' returns a structured JSON map of elements with numbered refs (preferred; pass interactive_only=true and/or scope_selector to keep output small — content-heavy pages emit hundreds of text-node refs otherwise). Refs are renumbered on EVERY map call and go stale when the page changes — re-map before reusing them. 'html' returns raw DOM HTML (large — use only when map is insufficient). 'state' returns lightweight metadata (URL, title, scroll, viewport). 'find_element' locates an element by ref/selector and returns its exact CSS pixel coordinates — this is the RECOMMENDED way to get click targets (do NOT estimate coordinates from screenshots). 'app_info' returns app name, version, OS, windows, and monitors.",
    {
